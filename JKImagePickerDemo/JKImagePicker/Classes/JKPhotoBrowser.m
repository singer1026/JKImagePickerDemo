...
{
    if (!self.checkButton.selected&&self.pickerController.selectedJKAssetArray.count>=self.pickerController.maximumNumberOfSelection) {
    ...
    if (animated){
        [UIView animateWithDuration:0.3f
                         ...
                         completion:^(BOOL finished) {
                             ...
                             if ([self.delegate respondsToSelector:@selector(photoBrowserClose:)]) {
                                 [self.delegate photoBrowserClose:self];
                             }
                         }];
    }
    else{
...
{
    for (JKAssets *asset in self.pickerController.selectedJKAssetArray) {
...
- (void)updateStatus
{
    ...
    [self.checkButton setTitle:asset.num forState:UIControlStateNormal];
}
